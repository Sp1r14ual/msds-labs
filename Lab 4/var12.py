...
class SystemCache:
    ...
    def precompute(self):
        ...
        dP_kk_2 = np.zeros((2, 2))  # Необходимо отслеживать производную и по 2-му параметру
        ...
        for k in range(N):
            ...
            K_A = np.vstack((K_k, dK_k_1, dK_k_2))
            ...
            # Инновация (ВНИМАНИЕ: dR/dtheta_2 = 1.0)
            B_k1 = (H @ P_k1_k @ H.T)[0, 0] + R
            dB_k1_1 = (H @ dP_k1_k_1 @ H.T)[0, 0] + 0.0
            dB_k1_2 = (H @ dP_k1_k_2 @ H.T)[0, 0] + 1.0  # Учитываем зависимость R от theta2
            ...
            dK_k1_2 = dP_k1_k_2 @ H.T / B_k1 - P_k1_k @ H.T / (B_k1**2) * dB_k1_2
            ...
            for i in range(s):
                for j in range(s):
                    ...
                    db_i = dB_k1_1 if i == 0 else dB_k1_2
                    db_j = dB_k1_1 if j == 0 else dB_k1_2
...
def calc_total_M(U_matrix, p):
    ...
    # Добавляем регуляризацию, чтобы избежать SingularMatrix error
...
def run_direct_procedure(opt_type="A"):
    ...
    MAX_ITER = 3 # Количество итераций пересчета P и U
...
if __name__ == "__main__":
    print("Выполнение прямой процедуры синтеза (Вариант 12)... Пожалуйста, подождите.\n")
    print("Примечание: из-за неустойчивости матрицы F значения критериев (след/детерминант)\n"
          "имеют экспоненциальный масштаб.\n")
